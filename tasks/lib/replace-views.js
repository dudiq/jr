...
function getReplaceContent(grunt, excludes, where, prefix) {
    ...
    for (var i = 0, l = templates.length; i < l; i++) {
        ...
        var fileKey = fileName.replace(/.html$/i, "");
        if (fileKey.indexOf(prefix) == 0) {
            fileKey = fileKey.substring(prefixLen);
        }
...
module.exports = function (grunt, options, units) {
    ...
    var toReplace = getReplaceContent(grunt, excludes, viewsSrc, options.prefix);
