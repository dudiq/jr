...
module.exports = function (grunt) {
    ...
    function collectFiles(filesForProcess, len, outputDir, files, outExt) {
        for (var i = 0, l = files.length; i < l; i++){
            var file = files[i];
            var outFilePath = outputDir + file.substr(len);
            var outFile = getPathWithoutExt(outFilePath) + '.' + outExt;
            var item = {
                fromFile: file,
                outFile: outFile
            };
    ...
    grunt.registerMultiTask('compileScss', 'Compile Sass to CSS', function () {
        var done = this.async();
        ...
        syncFormEach(inputDirs, function (val) {
            if (val){
                ...
                collectFiles(filesForProcess, len, outputDir, files, outExt);
            }
        });
