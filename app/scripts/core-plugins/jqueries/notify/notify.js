...
(function(){
    ...
    var template = templater.get('scripts/core-plugins/jqueries/notify/notify');
    ...
    var icons = config.notify ? config.notify.icons || {} : {};
    ...
    notify.show = function(message, icon, params){
        ...
        item = item.replaceAll('{{set-icon}}', icons[icon] || "");
        item = item.replaceAll('{{set-close-btn}}', icons.close || "");

        item = translate.text(item);
        var id = helper.guid();
        var el = $(item);
        el.data(CONST_ID, id);
