...
(function(){
    ...
    watchScope.getElements = function (scopeItem) {
        var ret = null;
        var item = scopeItem ? scopeItem[CONST_LINK] : null;
        if (item) {
            ret = [];
            helper.arrayWalk(item.pWatchers, function (item) {
                ret.push(item.getElement());
            });
        }
        return ret;
    };
